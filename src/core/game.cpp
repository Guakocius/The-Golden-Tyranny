...
namespace game {
    const std::string invalidChoice = "Invalid choice. Please try again.";
}
...
static void selectScreen(int choice,
    const nlohmann::json_abi_v3_12_0::json &classes,
    std::string playerName, Player player) {
        ...
        std::cout << "You have selected the " << selectedClass["name"]
                  << " class:\n"
        ...
        selectScreen(choice, classes, playerName, player);
    }
}
...
void GameMenu::newGame() {
    ...
    std::cout << "For centuries, the Sun Elves of Solarion believed themselves "
                 "destined to rule. Under the divine mandate\n"
              << "of the Sun God, their golden banners swept across the "
                 "continent of Apexia, uniting the fractured realms\n"
              << "under a single banner. At the helm of this conquest stood "
                 "Sun Emperor Lotharian, a visionary ruler\n"
              << "whose relentless campaigns saw the collapse of every "
                 "kingdom, city-state, and freehold that once stood\n"
              << "defiant.\n\n"
              << "With the known world now under the Empire's dominion, the "
                 "Golden Century began—a time of\n"
              << "unparalleled wealth, knowledge, and progress. The marble "
                 "spires of Solarion gleamed in the eternal sun,\n"
              << "its scholars and artisans ushering in an age of "
                 "enlightenment. The Sun Elven aristocracy flourished,\n"
              << "basking in the empire's prosperity, their dominion "
                 "unchallenged... or so they believed.\n\n"
              << "Beyond the gilded walls of Solarion's grandeur, however, the "
                 "fires of rebellion burn. The conquered\n"
              << "races—humans, dwarves, orcs, and others—chafe under elven "
                 "rule. Forbidden from practicing their\n"
              << "ancient traditions, taxed beyond reason, and consigned to "
                 "servitude, they now rise in defiance. Across\n"
              << "the empire's vast lands, hidden insurgencies grow bolder, "
                 "striking at garrisons, sabotaging supply lines,\n"
              << "and rallying behind fallen kings and forgotten gods.\n\n"
              << "For the first time in a century, the Sun Elves face a war "
                 "not of conquest, but of survival. The empire\n"
              << "stands at a crossroads—will it tighten its grip to crush the "
                 "uprisings, or will its hubris lead to a reckoning\n"
...
void GameMenu::exploration() { eh.explore(e); }
